...
//* * Retorna o total de títulos que foram baixados este mês */

export const CountPaidThisMonth = async (app: FastifyInstance) => {
  app.get(
    ...
    async (req, reply) => {
      ...
      try {
        const sql = `select count(*) as TOTAL from btitur where EXTRACT(MONTH FROM DATA) = EXTRACT(MONTH FROM CURRENT_DATE)
